...
export default function ProdutosPage() {
  ...
  async function carregar() {
    ...
    const { data } = await supabase.from("produtos")
      .select("id, nome, descricao, unidade, categoria, preco_custo, estoque_minimo, destaque, ativo, created_at")
      .order("nome");
    if (data) {
  ...
  const filtrados = useMemo(() => {
    ...
    return produtos
      ...
      .filter((p) => !q || [p.nome, p.categoria ?? "", p.unidade].join(" ").toLowerCase().includes(q));
