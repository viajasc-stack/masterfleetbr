-- Permite concluir/finalizar OS via painel sem exigir foto final do odômetro,
-- mantendo a exigência apenas quando a ação for feita por motorista autenticado.
...
DECLARE
  ...
  v_actor_is_motorista boolean := false;
BEGIN
  IF TG_OP <> 'UPDATE' OR v_new_status = v_old_status THEN
    RETURN NEW;
  END IF;

  -- Identifica se o ator autenticado é motorista (compatível com bases antigas/novas)
  ...
  -- evidência final somente se km_final < último km_final já registrado E ação feita por motorista
  IF v_new_status IN ('concluida', 'finalizada') THEN
    ...
    IF v_actor_is_motorista
       AND v_km_final_abaixo_ultimo
