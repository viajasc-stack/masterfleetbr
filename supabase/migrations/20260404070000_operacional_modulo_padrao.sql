-- =============================================================================
-- Consolidação do módulo padrão "Operacional"
-- =============================================================================

-- O módulo operacional passa a representar o pacote principal da operação:
-- dashboard, OS, veículos, motoristas, fretamentos, contratos,
...
VALUES (
  'operacional',
  'Operacional',
  'Dashboard, OS, veículos, motoristas, fretamentos, contratos, passageiros e clientes',
  true,
  'operacional',
  27600,
  50,
  ...
  jsonb_build_object(
    'default_module', true,
    'bundle_of', jsonb_build_array('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas')
  )
)
...
SET
  ...
  updated_at = now()
WHERE codigo IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas');
...
SET
  ...
  ended_at = NULL,
  preco_centavos = NULL,
  origem = CASE WHEN origem IN ('manual', 'manual_master') THEN origem ELSE 'sistema_base' END,
  updated_at = now()
WHERE modulo_codigo = 'operacional'
  AND origem IN ('sistema_base', 'migracao_operacional');

UPDATE public.empresa_modulos
SET
  status = 'cancelada',
  ended_at = COALESCE(ended_at, now()),
  origem = 'migracao_operacional',
  updated_at = now()
WHERE modulo_codigo IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas')
  AND status = 'ativa';
...
AS $$
  ...
  LEFT JOIN public.empresa_modulos em
   ...
   AND em.status = 'ativa'
  WHERE mg.codigo NOT IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas')
...
AS $$
  SELECT
    ...
    d.venda_ativa,
    d.ativo_empresa,
    (d.codigo IN ('operacional', 'configuracoes', 'usuarios', 'suporte')) AS base_obrigatoria
...
BEGIN
  ...
  IF p_modulo_codigo IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas') THEN
    RAISE EXCEPTION 'legacy_module_not_available';
  END IF;

  IF p_modulo_codigo IN ('operacional', 'configuracoes', 'usuarios', 'suporte') AND NOT p_ativo THEN
...
BEGIN
  ...
  WHERE empresa_id = p_empresa_id
    AND modulo_codigo NOT IN ('operacional', 'configuracoes', 'usuarios', 'suporte')
    AND status = 'ativa'
    AND NOT (modulo_codigo = ANY(COALESCE(p_modulos, ARRAY[]::text[])));

  FOREACH v_mod IN ARRAY COALESCE(p_modulos, ARRAY[]::text[]) LOOP
    IF v_mod IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas') THEN
      v_mod := 'operacional';
    END IF;
    ...
    IF EXISTS (
      ...
      WHERE codigo = v_mod
        AND codigo NOT IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas')
    ) THEN
      INSERT INTO public.empresa_modulos (empresa_id, modulo_codigo, status, origem, ended_at)
...
BEGIN
  ...
  WHERE m.codigo NOT IN ('dashboard', 'ordens_servico', 'clientes', 'veiculos', 'motoristas')
